...
% output: ACC Purity NMI ARI MIhat
if size(Y,2) ~= 1
    Y = Y';
end
if size(predY,2) ~= 1
...
NMI = NormalizedMutualInformation(Y,predY);

ARI = adjrandindex(predY, Y);


% result
result = [ACC Purity NMI ARI MIhat];
...
function adjrand = adjrandindex(v,u)
